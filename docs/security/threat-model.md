...
| Event loss or duplicate publish after ledger commit | Financial mutation commits but downstream consumer never receives event, or two workers publish the same event concurrently | Reporting, notifications, or partner systems diverge from ledger | Transactional outbox, publishing claim leases, stale-lease recovery, retry backoff, dead-letter state, operator retry | Add external broker publisher, event schema validation, DLQ replay approvals, publish latency SLO |
...
| Ledger mutation outside domain service | Developer bypasses journal poster and writes ledger rows incorrectly | Unbalanced or unexplained financial history | Ledger validations, database constraints, append-only Active Record guards, service tests, ADR guidance | Add database triggers or stricter write APIs if multiple writers appear |
