...
- Payout settlement before `settlement_due_on` is an exception path, not a plain `force` flag. PostgreSQL rejects early settled payouts unless the row references an approved maker-checker `operator_approval` with action `payout.settle_early` for the same payout.
- Refund rows are also guarded against Pix over-refund in PostgreSQL. Direct refund writes lock the source Pix payment, settled refunds must match the Pix organization, wallet, and currency, cumulative settled refund amount cannot exceed the original Pix amount, and a Pix payment with settled refund evidence cannot be directly reversed.
- Split entries are not only service-validated: PostgreSQL requires their organization, currency, amount sum, journal evidence, source-vs-destination separation, and unique destination wallet per split payment.
...
- `balance_projections` are derived read models and can be rebuilt from wallet liability ledger accounts. PostgreSQL prevents projection rows whose wallet, organization, or currency do not match, and amount changes require a local write context set by ledger posting or projection rebuild code.
...
- `outbox_events` preserve integration delivery evidence. PostgreSQL blocks direct published inserts, envelope mutation, deletion, any mutation after an event is published, and events whose aggregate type, aggregate row, organization, event type, command idempotency key, or key payload identifiers do not match a real financial fact. Published legacy command-identity gaps are accepted only through immutable exception rows tied to the original payload hash and expected command key. Terminal MED events additionally require payload evidence for `operator_approval_id`, `refund_id` when refunded, terminal status, and `resolved_at`.
