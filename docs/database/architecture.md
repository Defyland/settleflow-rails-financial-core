...
- `outbox_events` preserve integration delivery evidence. PostgreSQL blocks direct published inserts, envelope mutation, deletion, any mutation after an event is published, and events whose aggregate type, aggregate row, organization, event type, or key payload identifiers do not match a real financial fact. Terminal MED events additionally require payload evidence for `operator_approval_id`, `refund_id` when refunded, terminal status, and `resolved_at`.
