...
5. Recreate schema if needed:

```bash
bin/rails clickhouse:create_schema
```

6. Backfill published outbox events:

```bash
bin/rails clickhouse:backfill[10000]
```

7. Compare processed event count with published outbox count.
8. If sync fails with `Processed event does not match outbox event`, preserve the row as evidence and investigate outbox/process drift before retrying.
9. Do not rewrite PostgreSQL financial rows to repair analytics.
